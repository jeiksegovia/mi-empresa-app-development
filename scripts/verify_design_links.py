#!/usr/bin/env python3
"""
Link Verification Script for .superdesign/design_iterations/

Verifies that all internal HTML links in design files point to existing files.
This prevents broken navigation in the design iteration files.

Usage:
    python scripts/verify_design_links.py
"""

import os
import re
from pathlib import Path

# Configure the design iterations directory
DESIGN_DIR = Path("/Users/jeik/Documents/obsidian/work-vault/mi-empresa-app/.superdesign/design_iterations")

def verify_links():
    """
    Scan all HTML files in the design iterations directory and verify that
    all href links to .html files point to existing files.
    """
    # Get all HTML files (excluding old/deprecated)
    html_files = sorted([
        f for f in DESIGN_DIR.glob("*.html")
        if "old" not in f.name and "deprecated" not in f.name
    ])

    print("=" * 80)
    print("LINK VERIFICATION REPORT")
    print("=" * 80)
    print()

    all_broken = []
    all_ok = True

    for html_file in html_files:
        content = html_file.read_text(encoding='utf-8')

        # Find all href links to .html files
        links = re.findall(r'href="([^"]*\.html)"', content)

        # Remove duplicates and filter out external links
        links = sorted(set([l for l in links if not l.startswith('http')]))

        if links:
            print(f"\n📄 {html_file.name}")
            print("-" * 80)

            for link in links:
                target_file = DESIGN_DIR / link
                if target_file.exists():
                    print(f"  ✓ {link}")
                else:
                    print(f"  ✗ MISSING: {link}")
                    all_broken.append(f"{html_file.name} -> {link}")
                    all_ok = False

    print("\n" + "=" * 80)
    if all_ok:
        print("✅ ALL LINKS ARE VALID!")
    else:
        print(f"❌ FOUND {len(all_broken)} BROKEN LINKS:")
        for broken in all_broken:
            print(f"  • {broken}")
    print("=" * 80)

    return all_ok


if __name__ == "__main__":
    success = verify_links()
    exit(0 if success else 1)
